...
//		clist.addAll(zc4Set);
		...
		// 2
		final Set<Class<?>> clsSet = new HashSet<>();
		final Set<Class<?>> allClass = G.getAllClass();
		final Set<Class<?>> all = allClass.parallelStream()
				.filter(cls -> cls!=null)
			.filter(cls -> cls.isAnnotationPresent(ZComponent.class)
					|| cls.isAnnotationPresent(ZService.class)
					|| cls.isAnnotationPresent(ZController.class)
					|| cls.isAnnotationPresent(ZRestController.class))
			.collect(Collectors.toSet());
		clsSet.addAll(all);
		clsSet.addAll(APT.getAllClass());
		final List<Class<?>> clist = new ArrayList<>(clsSet);
