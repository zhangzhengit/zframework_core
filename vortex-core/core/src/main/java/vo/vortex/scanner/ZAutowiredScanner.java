...
//		for (final Class annoCls : cA) {
//			final Set<Class<?>> t = ClassMap.scanPackageByAnnotation(annoCls, startupInfo.getPackageNameArray());
//			zcSet.addAll(t);
//		}
		...
		final Set<Class<?>> al = load.stream()
				.filter(cls -> cls!=null)
				.filter(cls -> cls.isAnnotationPresent(ZService.class)
						 || cls.isAnnotationPresent(ZComponent.class)
						 || cls.isAnnotationPresent(ZController.class)
						 || cls.isAnnotationPresent(ZRestController.class)
						 || cls.isAnnotationPresent(ZConfiguration.class)
						 || cls.isAnnotationPresent(ZAOP.class)
						)
		.collect(Collectors.toSet());
		zcSet.addAll(al);
		zcSet.addAll(APT.getAllClass());
...
//		final Set<Class<?>> zcSet = ClassMap.scanPackageByAnnotation(annoClass, startupInfo.getPackageNameArray());
		...
		for (final Class<?> cls : zcSet) {
			Object o2 = null;
			// 2
			final boolean isZAOP = cls.isAnnotationPresent(ZAOP.class);
			if (isZAOP) {
			...
			// 1
//			if ((cls == ZRestController.class)
//					|| (cls == ZController.class)
//					|| (cls == ZComponent.class)
//					|| (cls == ZService.class)
//					|| (cls == ZConfiguration.class)
//					) {
//				o2 = ZContext.getBean(cls);
//			} else if (cls == ZAOP.class) {
//				o2 = ZSingleton.getSingletonByClass(cls);
//			}
	...
	public static void after(final ZApplicationStartupInfo startupInfo) {
